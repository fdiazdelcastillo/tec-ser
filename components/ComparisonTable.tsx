...
const ComparisonTableCard: Component<ComparisonTableCardProps> = (props) => {
  return (
    <div className="flex flex-shrink-0 flex-col py-14 px-6 max-sm:px-3 gap-7 max-md:gap-4 text-text items-center font-mont font-semibold text-base max-md:text-sm border-l-2 border-background xl:w-[195px] lg:w-[189px] max-md:w-[177px] relative">
      <div
        style={{
      ...
      <div className="w-44 max-md:w-36 mb-[50px] h-8">
        <div className="font-bold text-xl max-md:text-lg sm:text-base text-primary text-center">
          {props.title}
        </div>
      </div>
...
const ComparisonTable: Component<ComparisonTableProps> = (props) => {
  ...
  return (
    <div className="flex flex-row flex-grow rounded-[20px] border-solid border-background border-2 [background-image:radial-gradient(53%_52%_at_top,rgba(255,255,255,0.15),rgba(255,255,255,0))] max-xl:overflow-x-scroll scroll-smooth overflow-hidden">
      <div className="flex flex-col py-14 px-10 max-sm:px-6 gap-7 max-md:gap-4 text-text font-mont text-base max-md:text-sm font-semibold [background-image:radial-gradient(143%_46%_at_bottom_left_,rgba(61,138,255,0.50),rgba(34,71,126,0.50),rgba(13,17,23,0.2))] max-lg:sticky -left-1 sm:sticky max-xl:bg-background z-[1]">
        <div
          style={{
            backgroundImage: "url(/noise.webp)",
            backgroundSize: "30%",
          ...
          className="absolute object-contain inset-0 w-full h-full  scale-y-100 transform opacity-[0.019]"
        ></div>
        <div className="mb-12 text-center text-xl max-md:text-lg sm:text-base text-primary font-bold ">
          Características
        </div>
